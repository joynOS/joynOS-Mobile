...
export default function Welcome() {
  ...
  useEffect(() => {
    if (response?.type === "success") {
      setIsLoading(true);
      navigation.navigate('PersonalityQuiz', {} as any);
      setIsLoading(false);
    }
  }, [response]);
          ...
          <View style={styles.card}>
            {!showPhoneAuth ? (
              ...
              <PhoneAuth
                phone={phone}
                setPhone={setPhone}
                onRequestPhone={async (p) => {
                  await requestPhoneCode(p);
                }}
                onVerifyPhone={async (p, c, n) => {
                  await verifyPhoneCode(p, c || "4444", n);
                  navigation.navigate("PersonalityQuiz", { phone: p });
                }}
